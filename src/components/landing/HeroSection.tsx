...
const HeroSection = ({ onStart, onDemo }: HeroSectionProps) => (
  <div className="text-center mb-16 md:mb-24 animate-fade-in">
    <div className="inline-flex items-center space-x-2 bg-purple-500/10 backdrop-blur-sm border border-purple-500/20 rounded-full px-4 md:px-6 py-2 mb-6 md:mb-8 animate-subtle-pulse">
      <Zap className="w-3 h-3 md:w-4 md:h-4 text-purple-400 animate-soft-bounce" />
    ...
    <h2 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 md:mb-8 leading-tight px-2 animate-gentle-float">
      Master Your
      <span className="block bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent animate-subtle-pulse"> 
        Coding Journey
      </span>
    </h2>
    ...
    <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center items-center px-4 animate-fade-in" style={{ animationDelay: '600ms' }}>
      ...
      >
        <Rocket className="w-4 h-4 md:w-5 md:h-5 mr-2 md:mr-3 group-hover:animate-soft-bounce" />
      ...
      >
        <Sparkles className="w-4 h-4 md:w-5 md:h-5 mr-2 md:mr-3 group-hover:animate-subtle-pulse" />
        Watch Demo
      </Button>
    </div>
  </div>
